...
(function() {
  ...
  Locomotive.Views.EditableElements.IframeView = (function(superClass) {
    ...
    IframeView.prototype.on_load = function(event) {
      var path;
      if ((path = this.edit_view_path(event)) != null) {
        ...
        this.build_and_render_page_view();
      }
      return false;
    };

    IframeView.prototype.edit_view_path = function(event) {
      var $document, e;
      try {
...
(function() {
  ...
  Locomotive.Views.Inputs.ArrayView = (function(superClass) {
    ...
    ArrayView.prototype.begin_add_item = function(event) {
      var data, item_id;
      event.stopPropagation() & event.preventDefault();
      item_id = this.$new_input.val();
      if (!this.is_unique() || item_id === null) {
        return;
      }
      data = {
        id: item_id
      };
      data[this.$new_input.attr('name')] = item_id;
      return $.ajax({
  ...
  Locomotive.Views.Inputs.Rte.FileView = (function(superClass) {
    ...
    FileView.prototype.show = function(state) {
      ...
      if (state === false) {
        ...
        this._input_el('alt').val($image.attr('alt'));
...
(function() {
  ...
  Locomotive.Views.Inputs.Rte.LinkView = (function(superClass) {
    ...
    LinkView.prototype.show = function(state) {
      ...
      this._input_el('target', 'select').select2();
      this._input_el('target', 'select').data('select2').$dropdown.addClass('rte-select2-dropdown');
      if (state === false) {
...
(function() {
  ...
  Locomotive.Views.Inputs.RteView = (function(superClass) {
    ...
    RteView.prototype.register_editor_events = function() {
      ...
      return this.editor.on('change_view', this.resize);
    };
    ...
    RteView.prototype.resize = function() {
      var $el, $iframe, $inputs, $target, $textarea, $wrapper, delta_height, height, i, j, len, len1, ref, ref1, results, results1, wrapper_max_height;
      $iframe = this.$('.form-wrapper .wysihtml5-sandbox');
      $textarea = this.$('.form-wrapper textarea');
      ...
      if ($inputs.hasClass('expanded')) {
        ...
        for (i = 0, len = ref.length; i < len; i++) {
          ...
          if ($target.data('height') == null) {
            $target.data('height', $target.css('height'));
          }
          results.push($target.css('min-height', height));
        }
        return results;
      } else {
        ref1 = [$iframe, $textarea];
        results1 = [];
        for (j = 0, len1 = ref1.length; j < len1; j++) {
          $el = ref1[j];
          results1.push($el.css('min-height', $el.data('height')));
        }
        return results1;
      }
    };
